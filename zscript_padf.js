...
let wave = 1;
let waveTimer = 0;
let waveInterval = 15 * 60; // 15 secondes
...
const COSTS = {
  ...
  arrowTurret: { money: 25, range: 260, dmg: 40, fireRate: 60 },

iceTurret: { money: 100, range: 200, slow: 0.5, duration: 120, fireRate: 60 }, 

  laserTurret: { money: 50, range: 100, dmg: 5, fireRate: 5 }
};
...
const UPGRADE_DATA = {
  ...
  arrow: [
    { cost: 150, dmg: 60, range: 300, fireRate: 30, texture: 'img/turret_arrow2.png' },
  ],

  laser: [
    { cost: 250, dmg: 15, range: 150, fireRate: 5, texture: 'img/turret_laser2.png' },
  ],

  ice: [
    { cost: 200, slow: 0.3, range: 220, duration: 120, texture: 'img/turret_ice2.png' },
  ]
};
...
function openUpgradeMenu(t) {
  ...
  if (!upgrades) {
    upgradeInfo.textContent = "Cette tourelle ne peut pas être améliorée.";
    upgradeButton.style.display = 'none';
  } else {
    const lvl = t.level || 0;
    if (lvl >= upgrades.length) {
      upgradeInfo.textContent = "Tourelle déjà au niveau maximum.";
      upgradeButton.style.display = 'none';
    } else {
      const u = upgrades[lvl];
      upgradeInfo.textContent = `Améliorer niveau ${lvl+1} : ${u.cost}$`;
...
for (let z of zombies) {
  ...
  // ==== Gestion des vagues ====
  if (frame < 60) return; // délai 3 secondes avant première vague
  ...
  if (!waveInProgress) {
    ...
    if (zombies.length === 0) {
      ...
      wave++;
      console.log(`✅ Vague ${wave-1} terminée. Prochaine dans ${waveInterval/60}s`);
    }
  }
...
for (let t of turrets) {
  ...
  // wave info
  ctx.font='16px Arial'; ctx.fillStyle='white'; ctx.textAlign='center';
  if (waveInProgress) ctx.fillText(`Vague ${wave}`, canvas.width/2, 30);
  else {
    const timeLeft = Math.max(0, Math.ceil((waveInterval-waveTimer)/60));
    ctx.fillText(`  vague dans ${timeLeft}s`, canvas.width/2,30);
  }
